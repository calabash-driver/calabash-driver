...
public class ListItemImpl extends RemoteObject implements ListItemSupport {
	...
	public void press() {
		if (by instanceof By.Index) {
			executeCalabashCommand(CalabashCommands.PRESS_LIST_ITEM,
					by.getIndentifier(),"0");
		} else if (by instanceof By.ContentDescription) {
			executeCalabashCommand(CalabashCommands.PRESS_LIST_ITEM_TEXT,
					by.getIndentifier());
	...
	public void longPress() {
		if (by instanceof By.Index) {
			executeCalabashCommand(CalabashCommands.LONG_PRESS_LIST_ITEM,
					by.getIndentifier());
		} else {
			throw new IllegalArgumentException("By not available.");
		}
