...
public abstract class CalabashAndroidDriver {
	...
	public CalabashAndroidDriver(String remoteURL,
	...
	private Session start() throws Exception {
		...
		WebDriverLikeRequest request = new WebDriverLikeRequest("POST",
				"/session", payload);
	 ...
	 */
	public void quit() {
		WebDriverLikeRequest request = new WebDriverLikeRequest("DELETE",
				"/session/" + session.getSessionId(), new JSONObject());
		try {
			execute(request);
		} catch (Exception e) {
