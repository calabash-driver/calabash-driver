...
public class EndToEndTests {
	...
	public void startServer() throws Exception {
		server = new CalabashAndroidServer(
				CalabashNodeConfiguration
						.readConfig("src/test/resources/driverConfig.json"));
		server.start();
	}
	...
	public void scriptStartsAndRegisterToServer() {
		...
		try {
			...
			driver = new RemoteCalabashAndroidDriver("http://" + host + ":"
					+ port + "/wd/hub", caps);

			System.out.println("created session: "
					+ driver.getSession().getSessionId());
			System.out.println("session cap: "
					+ driver.getSession().getActualCapabilities());
			 ...
			 * Assert.assertEquals(target.getReference(), "1"); target =
			 * (RemoteUIATarget) driver.getLocalTarget();
