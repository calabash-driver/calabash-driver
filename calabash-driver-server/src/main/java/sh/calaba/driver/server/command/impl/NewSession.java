...
public class NewSession extends BaseCommandHandler {
	...
	public WebDriverLikeResponse handle() throws Exception {
		JSONObject desiredCapabilities = getRequest().getPayload()
				.getJSONObject("desiredCapabilities");
		System.out.println("desiredCapabilities: " + desiredCapabilities);
		CalabashCapabilities capabilities = CalabashCapabilities
				.fromJSON(desiredCapabilities);

		JSONObject json = new JSONObject();
		String sessionID = getCalabashProxy().initializeSessionForCapabilities(
				capabilities);
		...
		System.out.println("Created session ID in response: "
				+ r.getSessionId());
		return r;
	}
}
